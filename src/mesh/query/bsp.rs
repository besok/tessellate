...
impl BSPTree {
    ...
    pub fn mesh(&self, attrs: Attributes) -> Mesh {
        let polygons: HashSet<Polygon> = self
            .iter_preorder()
            .flat_map(|node| node.polygons().clone())
            .collect();
        Mesh::from_polygons(polygons.into_iter().collect(),attrs)
    }

    /// Generates a mesh from the BSP tree planes
    pub fn plane_meshes(&self, size: f32, attrs: Attributes) -> Vec<Mesh> {
        let mut meshes = vec![];
        for node in self.iter_preorder() {
            if let BSPNode::Node { plane, .. } = node {
                meshes.push(plane.to_mesh(size, attrs.clone()));
            }
        }
        meshes
    }
